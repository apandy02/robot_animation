...
def train(data):
    ...
    mean_pg_loss, mean_v_loss, mean_entropy_loss = 0, 0, 0  # noqa
    mean_old_kl, mean_kl, mean_clipfrac = 0, 0, 0  # noqa
    ...
    with profile.train_misc:
        ...
        if profile.update(data) or ("episode_return" in data.stats or done_training):
            print_dashboard(
                config.env,
                data.utilization,
                data.global_step,
            ...
            if (
                ...
                and (time.time() - data.last_log_time > 60)  # or "episode_return" in data.stats)
            ):
...
def print_dashboard(
    ...
    log.add_column(f"{c1}Losses", justify="left", width=16)
    log.add_column(f"{c1}Value", justify="right", width=8)
    for metric, value in losses.items():
        log.add_row(f"{c2}{metric}", f"{b2}{value:.3f}")

    monitor = Table(box=None, expand=True, pad_edge=False)
    monitor.add_row(s, p, log)
